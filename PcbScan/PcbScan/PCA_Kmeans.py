...
# def clustering_to_mse_values(change_map, image1, image2, n_clusters):
#     mse = [0] * n_clusters
#     size = [0] * n_clusters

#     for i in range(change_map.shape[0]):
#         for j in range(change_map.shape[1]):
#             label = change_map[i, j]
#             mse[label] += np.sum((image1[i, j] - image2[i, j]) ** 2)
#             size[label] += 1

#     return [(mse[k] / (255**2)) / size[k] if size[k] != 0 else 0 for k in range(n_clusters)], size
